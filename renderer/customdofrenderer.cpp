...
void CustomDofRenderer::renderThread(const Scene *scene, Camera *camera, Texture *image, int width, int widthStep, int widthOffset, int height, int heightStep, int heightOffset, std::atomic<int> *k, int const stepSize,
  ...
  for (int y = heightOffset; y < image->height(); y += heightStep) {
    for (int x = widthOffset; x < image->width(); x += widthStep) {
      ...
      // For each pixel new seed
      uint32_t pixelIndex = y * image->width() + x;

      for (int xs = 0; xs < superSamplingFactor; ++xs) {
          for (int ys = 0; ys < superSamplingFactor; ++ys) {

              Ray ray = camera->createRay(((xs * samplingStep + x) / width * 2 - 1), -((ys * samplingStep + y) / height * 2 - 1) * aspectRatio, pixelIndex);
...
Texture CustomDofRenderer::renderImage(Scene const &scene, Camera &camera, int width, int height) {
